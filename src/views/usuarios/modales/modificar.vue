<template>
  <div>
    <modal ref="modal" titulo="Modificar usuario" :impedir-close="impedirClose">
      <el-form label-width="170px" v-loading="loading" :rules="formRules">
        <!-- Nombre -->
        ...
        <el-form-item label="Rol asignado" prop="rolAsignado">
          <el-input v-model="rolBD" disabled></el-input>
        </el-form-item>
        ...
        .then((response) => {
          ...
          if (respuestaApi.code == 200) {
            ...
            this.roles.forEach((elemento) => {
              if (elemento.id == respuestaApi.data.tipoUsuario) {
                this.rolBD = elemento.nombre
              }
            })

            // this.rolBD = respuestaApi.data.tipoUsuario;

...
export default {
	name: 'ModalEditarBorrador',
	components: { ModalBuscarProductoDeposito },
	data() {
		return {
			form: {
				nombreObra: null,
	...
	methods: {
		...
		async cantidadMeses() {
			...
			let meses = fechaaPresupuestar1.getMonth() - fechaaPresupuestar0.getMonth() + 12 * (fechaaPresupuestar1.getFullYear() - fechaaPresupuestar0.getFullYear()) + 1;
			this.form.mesesaPresupuestar = meses;

			let mostrarFechaIncio = fechaaPresupuestar0.getDate() + "/" + (fechaaPresupuestar0.getMonth() + 1) + "/" + fechaaPresupuestar0.getFullYear();
			let mostrarFechaFin = fechaaPresupuestar1.getDate() + "/" + (fechaaPresupuestar1.getMonth() + 1) + "/" + fechaaPresupuestar1.getFullYear();
		...
		activar2yTraerProveedoresxRubro() {
			...
			this.arrayProductosAComprar.forEach((elemento) => {
				if (elemento.cantidadDeposito == null) {
					b = 1;
				}
			});
		...
		async obtenerProveedorxRubro() {
			...
			let params = { arrRubrosAComprar: JSON.stringify(this.arrayRubrosAComprarEnviar) };
...
<template>
	<div>
		...
		>
			...
			>
				<el-step title="Obra" />
				<el-step title="Materiales" />
				<el-step title="Proveedores" />
			...
			>
				<div
					v-for="(item, index) in arrayRubrosAComprar"
					:key="index"
				>
					<el-checkbox
					...
					<el-col :span="12">
						<div class="block">
							<span
								...
								class="demonstration"
							>Fecha límite de carga proveedor</span>
							<el-date-picker

			...
			this.datosAPI.forEach((elemento) => {
				...
				if (arrayProductosProveedor.length > 0) {
					let fila = {
						...
						proveedor_nombre: elemento.proveedor_nombre,
						proveedor_rubro_id: elemento.proveedor_rubro_id,
						productos: JSON.stringify(arrayProductosProveedor),
				...
				<div style="display: flex">
					<div style="margin: auto">
						<h3 style="text-align: center">
							Existe un borrador generado para esta comparativa
						</h3>
						<h3 style="text-align: center">
							Indique si desea seguir con los datos guardados en el borrador o con los datos originales
						</h3>
					</div>
				</div>
						...
						>
							...
							>
								...
								<el-table
									...
									scrollbar-always-on
								>
									<el-table-column width="254px" />
									<el-table-column width="146px" />
									<el-table-column
										v-for="(item, index) in arrayInfoProveedores"
										:key="index"
										align="center"
									>
										<el-table-column
										...
										>
											<el-select
												v-model="item.proveedor_forma_de_pago_nombre"
												placeholder="Select"
												size="large"
												:disabled="deshabilitarPorOrdenCompra"
											>
								...
								>
									...
									<el-table-column
										v-for="(item, index) in arrayPrecioPPProveedores"
										:key="index"
										:label="item.titulo"
										align="center"
										width="99px"
									>
										...
										<span
											v-if="!isNaN(item.totalHomogeneo) && item.menorHomogeneo == 1"
											style="color: red"
										>
											<el-tag
												type="success"
										...
										<span
											v-if="!isNaN(item.totalHomogeneo) && item.menorHomogeneo == 0"
											style="color: black"
										>
											$ {{ new Intl.NumberFormat('de-DE').format(item.totalHomogeneo) }}
										</span>
